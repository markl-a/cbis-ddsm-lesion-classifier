...
**5-fold 交叉驗證（穩健性,最可辯護的數字）。** 跑完全部 5 個 patient-disjoint fold:

| fold | 0 | 1 | 2 | 3 | 4 | **mean ± std** | **5-model 集成** |
|---|---|---|---|---|---|---|---|
| test case-AUC | 0.780 | 0.776 | 0.768 | 0.755 | 0.756 | **0.767 ± 0.011** | **0.791** |

重點:**5 個 fold 全部都贏過 baseline 0.748** —— 提升是穩健的、不是幸運切分。穩健中心估計為
**0.767 ± 0.011**;交付的單一 fold `best.pt`(0.794)落在 fold 分布的**高端**,而 5-model 集成
(0.791,敏感度 0.689 / 特異度 0.719)藉由平均掉 fold 變異、回到 ~0.79。詳見
`artifacts/v2_effv2s288_5fold.json`(由 `train_v2.py --folds 0 1 2 3 4` 產生)。
