...
#
# > 註：本 notebook 每次執行都會**重新訓練一次**（DirectML 有隨機性），單次結果約落在
# > **0.77–0.80**。交付的 `best.pt` 是其中一次代表性訓練（test case-AUC 0.794，已做顯著性檢定）；
# > 最可辯護的數字是上面的 **5-fold mean±std / 集成**。下方 cell 輸出即為某一次實跑的真實結果。
#
# ## 這份作業最關鍵的一步：防資料洩漏
